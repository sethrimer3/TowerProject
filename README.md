...
- `src/tile-cache.ts`: caches layers of rarely-changing tiles (the stone or forest floor, and the decor's baked pixels) as one offscreen image per layer, covering the view plus a margin; it repaints only when the view leaves that area, the board or tile size changes, or tile art is still loading. Settings' **Battery saver** draws the board at 30 fps while nothing on it is moving.
