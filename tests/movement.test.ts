...
test("missing key walks to door, stops, and places feedback on the door", () => {
  ...
  assert.equal(g.save.history.length, 1);
});
...
test("fatal path resets to entrance, stops route, and cannot undo without Revive", () => {
  ...
  assert.equal(g.save.history.length, 0);
  assert.equal(g.undo(), false);
  assert.equal(g.save.essence, 1);
});
test("Revive restores pre-fatal state and rolls back pending rewards; next move forfeits it irreversibly", () => {
  ...
  g.run.changes["15,1"] = doom;
  const before = structuredClone(g.run);
  g.move(0, 1);
  assert.ok(g.save.revival);
  assert.equal(g.save.essence, 0);
  assert.ok(g.undo());
  ...
  g.summary = null;
  g.move(0, -1);
  assert.ok(g.save.revival, "Blocked move must not forfeit revival");
  g.move(0, 1);
  assert.equal(g.save.revival, null);
  assert.equal(g.save.essence, 1);
  g.undo();
  assert.equal(g.run.player.y, 0);
  assert.equal(g.save.revival, null);
...
test("undo and Revive persist safely across refresh", () => {
  ...
  loaded.save.revival = { snapshot: before, earned: 7 };
  loaded.save.history = [];
  const saved = decode(JSON.stringify(loaded.save));
  assert.ok(saved.revival);
  loaded = new Game(saved);
  assert.ok(loaded.undo());
  assert.equal(loaded.save.essence, 0);
  const corrupt = JSON.parse(JSON.stringify(saved));
  corrupt.history = [{ run: { player: {} }, best: 0 }];
  corrupt.revival = { snapshot: null, earned: 10 };
  assert.equal(decode(JSON.stringify(corrupt)).history.length, 0);
  assert.equal(decode(JSON.stringify(corrupt)).revival, null);
});
