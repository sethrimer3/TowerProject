...
export function defaults(): Save {
  return {
    ...
    settings: { density: 20, speed: 3, reduceMotion: false },
    run: null,
  };
}
...
export function decode(raw: string | null): Save {
  ...
  try {
    ...
    if (s?.version !== 1) return d;
    if (finite(s.best)) d.best = Math.floor(s.best);
    if (finite(s.essence)) d.essence = Math.floor(s.essence);
