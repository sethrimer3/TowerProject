...
import { gear, type Save, type Run, type Tile, type MoveSnapshot } from "./entities.ts";
...
export class Game {
  ...
  get undoCapacity(){return 1+this.save.upgrades.undos;}
  settleRevival(){if(this.save.revival){this.save.essence+=this.save.revival.earned;this.save.revival=null;}}
  snapshot():MoveSnapshot {return {run:structuredClone(this.run),best:this.save.best};}
  restore(snapshot:MoveSnapshot){this.run=structuredClone(snapshot.run);this.save.run=this.run;this.save.best=snapshot.best;this.world=new World(this.run.seed,this.run.changes,this.run.floor);this.route=[];this.auto=false;this.summary=null;this.paused=false;this.blocked.until=0;}
  undo(){
    if(this.save.revival){const snapshot=this.save.revival.snapshot;this.save.revival=null;this.save.history=[];this.restore(snapshot);this.feedback('Revived � fatal move undone');return true;}
    if(this.summary)return false;
    const snapshot=this.save.history.pop();if(!snapshot)return false;
    this.restore(snapshot);this.feedback('Move undone');return true;
  }
  reject(x:number,y:number,message:string){this.route=[];this.blocked={x,y,until:performance.now()+1000};this.message=message;}
  walkTo(x:number,y:number){if(this.paused||this.summary)return;this.auto=false;const route=routeTo(this,x,y);if(!route){this.reject(x,y,'No route to that space.');return;}this.route=route;this.message=route.length?'Walking to destination.':'Already here.';}
  routeStep(){const step=this.route.shift();if(!step)return false;const result=this.move(step.dx,step.dy,true);if(!result)this.route=[];return result;}
  newRun() {
    this.settleRevival();
    this.save.history=[];
    this.route=[];
  ...
  move(dx: number, dy: number, force = true) {
    ...
    if(!dest){this.reject(p.x,p.y,'That edge is closed.');return false;}
    const {x,y}=dest,t=this.world.tile(x,y);
    if(t.kind==='wall'){this.reject(x,y,'A wall blocks the way.');return false;}
    if(t.kind==='door'&&!p.keys[t.color!]){this.reject(x,y,`Requires a ${t.color} key.`);return false;}
    if(t.kind==='enemy'&&!force&&!predict(p,t.enemy!).survivable)return false;
    this.settleRevival();
    ...
    if (t.kind === "enemy") {
      ...
      if (p.hp <= 0) {
        ...
        const summary={height:this.run.height,kills:this.run.kills,earned,reason:'Fallen in battle',dead:true};
        this.newRun();
        if(this.save.upgrades.revive)this.save.revival={snapshot:before,earned};
