...
function update() {
  ...
  undo.textContent = slice.revival
    ? "Revive"
    : `Undo (${slice.history.length}/${game.undoCapacity})`;
...
function renderPage() {
  ...
  if (tab === "upgrades") {
    ...
    el("upgrades").innerHTML =
      `<div class="page-title"><small>WHAT REMAINS WHEN YOU FALL</small><h2>A lasting legacy</h2><p>Spend Essence and Shards to strengthen future ascents. Wayfinder, Revive, and extra undos unlock immediately.</p></div><h3 class="section-title">✦ Essence upgrades · Delve</h3>${section("essence")}<h3 class="section-title">◆ Shard upgrades · Tower</h3>${section("shards")}<p class="hint">Retire a Delve to earn Essence, and a Tower ascent to earn Shards — only when you beat your prior best. Every kill grants XP toward permanent level bonuses, in either mode.</p>`;
  ...
  if (tab === "settings") {
    ...
    el("retire").onclick = () =>
      confirmAction(
        "Leave your mark?",
        `Retire at ${game.mode === "tower" ? "height" : "depth"} ${game.run.height}. You will receive ${game.mode === "tower" ? "Shards" : "Essence"} if this beats your best.`,
        "Retire ascent",
        () => {
...
function showSummary() {
  const s = game.summary!,
    currencyName = game.mode === "delve" ? "ESSENCE" : "SHARDS",
