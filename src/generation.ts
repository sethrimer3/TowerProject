...
export function generate(seed: number, index: number): Map<string, Tile> {
  ...
  for (let r = 0; r < 4; r++) {
    const y = 2 + r * 4;
    const left = 3 + Math.floor(rng() * 4),
      right = 23 + Math.floor(rng() * 4);
    for (let yy = y; yy < Math.min(y + 3, CHUNK); yy++)
      for (let x = left; x <= right; x++) set(x, yy, { kind: "floor" });
    for (const side of [-1, 1]) {
      ...
      set(x, y, {
        ...
        enemy: {
          name: types[tier],
          hp: 12 + tier * 16 + Math.floor((base + y) * 0.5),
          attack: 6 + tier * 4 + Math.floor((base + y) / 12),
          defense: 1 + tier * 2,
          tier,
        },
      });
      ...
      set(x, y + 1, {
        kind: rng() < 0.5 ? "potion" : rng() < 0.5 ? "attack" : "defense",
      });
...
export function validate(cells: Map<string, Tile>, base: number) {
  ...
  const q = [[START_X, base]];
  for (let i = 0; i < q.length; i++) {
    const [x, y] = q[i],
      k = point(x, y);
    if (seen.has(k) || !cells.has(k) || cells.get(k)!.kind === "wall") continue;
    seen.add(k);
    if (y === base + CHUNK - 1 && x === START_X) return true;
    for (const [dx, dy] of [
      [1, 0],
      [-1, 0],
      [0, 1],
